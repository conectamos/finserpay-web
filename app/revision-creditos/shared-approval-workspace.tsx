  ...
  return <main className={styles.main}>
    ...
    <div className={styles.toolbar}>
      ...
      <form className={styles.search} onSubmit={event => { event.preventDefault(); if (!busy) props.onSearch(searchText.trim()); }}>
        <label className="sr-only" htmlFor="shared-approval-search">Buscar por cliente, cédula, crédito, folio o aliado</label>
        <Input id="shared-approval-search" type="search" placeholder="Cliente, cédula, crédito o aliado" autoComplete="off" maxLength={100} value={searchText} disabled={busy} onChange={event => setSearchText(event.target.value)} />
    ...
    <div id="shared-approval-view" role="tabpanel" aria-labelledby={`shared-view-${view}`} className={`${styles.workspace} ${selectedId ? styles.hasSelection : ""}`}>
      <Card className={styles.queue} aria-label="Lista de expedientes">
        ...
        <div className={styles.listScroll}>
          ...
          <ul className={styles.list}>{props.items.map(item => <li key={item.id}><button type="button" aria-label={`Revisar crédito ${creditDisplayNumber(item)}`} aria-current={selectedId === item.id ? "true" : undefined} className={`${styles.row} ${selectedId === item.id ? styles.selected : ""}`} disabled={busy} onClick={() => props.onSelect(item.id)}>
            <strong>{item.clienteNombre}</strong><span className={styles.document}>Cédula: {item.clienteDocumento?.trim() || "No disponible"}</span><span className={styles.folio}>{creditDisplayNumber(item)}</span>{creditDisplayNumber(item) !== item.folio ? <span className={styles.folio}>Folio: {item.folio}</span> : null}<span>{item.aliadoNombre}{item.sedeNombre ? ` · ${item.sedeNombre}` : ""}</span><span className={styles.rowDate}>{date(item.fechaCredito)}</span>
      ...
      <div className={styles.detailColumn}>
        ...
        {props.loadingDetail && !detail ? <Card className={styles.placeholder}><LoadingState label="Cargando el expediente seleccionado..." /></Card> : props.detailError ? <Card className={styles.placeholder}><p role="alert">{props.detailError}</p><Button variant="secondary" disabled={busy} onClick={props.onRetryDetail}>Reintentar expediente</Button></Card> : detail ? <Card className={styles.dossier} aria-busy={props.loadingDetail} aria-label={`Expediente del crédito ${creditDisplayNumber(detail)}`}>
          {props.loadingDetail ? <p role="status" className="px-5 pt-3 text-sm text-[var(--fp-muted)]">Actualizando expediente...</p> : null}<div className={styles.dossierHeading}><div><p>{creditDisplayNumber(detail)}</p><h2>{detail.clienteNombre?.trim() || "No disponible"}</h2><span>{detail.aliadoNombre}{selectedItem?.id === detail.id && selectedItem.sedeNombre ? ` · ${selectedItem.sedeNombre}` : ""}</span><dl className={styles.equipmentReference}><div><dt>Referencia del equipo</dt><dd>{detail.referenciaEquipo?.trim() || "No disponible"}</dd></div></dl><p>{date(detail.fechaCredito)}</p></div><Badge tone={view === "approved" ? "positive" : "warning"}>{view === "approved" ? "Aprobado" : "Pendiente"}</Badge></div>
          <details className={styles.customer}><summary>Datos del cliente<ChevronDown size={16} aria-hidden="true" /></summary><dl>{[["Nombre", detail.clienteNombre], ["Cédula", detail.clienteDocumento], ["Correo", detail.clienteCorreo], ["Teléfono", detail.clienteTelefono], ["Departamento", detail.clienteDepartamento], ["Ciudad", detail.clienteCiudad], ["Dirección", detail.clienteDireccion]].map(([label, value]) => <div key={label} className={label === "Dirección" ? styles.customerAddress : undefined}><dt>{label}</dt><dd>{value?.trim() || "No disponible"}</dd></div>)}</dl></details>
