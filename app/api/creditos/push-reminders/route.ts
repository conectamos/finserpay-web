...
import { dispatchCreditPushReminders } from "@/lib/credit-push-reminders";
import { isAdminRole } from "@/lib/roles";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

type PushReminderRequestBody = {
...
async function readBody(req: Request): Promise<PushReminderRequestBody> {
  try {
    return (await req.json()) as PushReminderRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(req: Request) {
  ...
  const report = await dispatchCreditPushReminders({
    dryRun: true,
