...
export async function DELETE(
  ...
  try {
    ...
    if (!adminCentral) {
      return NextResponse.json(
        { error: "Solo admin central FINSER PAY puede eliminar recaudos" },
        { status: 403 }
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.cajaMovimiento.deleteMany({
        where: {
          OR: [
            {
              descripcion: {
                contains: `${CREDIT_ABONO_CAJA_MARKER}${abono.id}`,
              },
            },
            {
              descripcion: {
                contains: `Anulacion de recaudo ${abono.id}`,
              },
            },
          ],
        },
      ...
      await tx.wompiPaymentIntent.deleteMany({
        where: { processedAbonoId: abono.id },
      });
    ...
    return NextResponse.json(
      { error: "No se pudo eliminar el recaudo" },
      { status: 500 }
    );
  }
}
