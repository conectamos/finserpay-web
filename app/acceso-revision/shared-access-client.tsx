  ...
  useEffect(() => {
    ...
    if (!request.current) {
      ...
      request.current = (async (): Promise<AccessResult> => {
        ...
        try {
          const response = await fetch("/api/public/approval-shared-access", {
            method: "POST",
    ...
    void request.current.then((next) => {
      ...
      if (next === "ready") {
        window.location.replace("/revision-creditos");
      } else {
        setResult(next);
      }
    });
  ...
  return (
    <main className="fp-ui-shell flex min-h-screen items-center justify-center px-4 py-10">
      <Card className="w-full max-w-lg p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-[var(--fp-muted)]">Enlace compartido para revisar créditos pendientes de aliados.</p>
        {result === null ? <div className="mt-6"><LoadingState label="Validando el enlace compartido..." /></div> : (
          <div className="mt-6 rounded-xl border border-[var(--fp-border)] p-4" role="alert">
            <p className="font-semibold">
              {result === "missing" ? "Necesitas el enlace compartido completo" : "Este enlace no permite el acceso"}
            </p>
            <p className="mt-2 text-sm text-[var(--fp-muted)]">
              {result === "missing"
                ...
                : "Solicita al administrador central que revise tu acceso y comparta un enlace vigente con el equipo autorizado."}
            </p>
          </div>
        )}
      </Card>
