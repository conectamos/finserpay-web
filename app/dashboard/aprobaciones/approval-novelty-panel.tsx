...
export default function ApprovalNoveltyPanel({ detail, disabled = false, compact = false, readOnly = false, onUpdated, onBusyChange }: Props) {
  ...
  return <Card role="region" aria-label="Novedades del expediente" className="min-w-0 space-y-4 p-4 sm:p-6">
    ...
    {!compact && !readOnly ? <p className="text-sm text-[var(--fp-muted)]">Indica al administrador del aliado qué debe corregir. Al guardar una foto nueva, el crédito vuelve a revisión del analista.</p> : null}
    ...
    {editing ? <div className="space-y-4 border-t border-[var(--fp-border)] pt-4">
      ...
      <div className="flex flex-wrap gap-3"><Button variant="secondary" disabled={saving || confirming} onClick={close}>Cancelar</Button><Button disabled={disabled || saving || confirming || !valid} onClick={() => setConfirming(true)}>{lockedDraft ? "Reintentar confirmación" : "Registrar novedad"}</Button></div>
    </div> : null}
