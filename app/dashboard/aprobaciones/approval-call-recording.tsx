...
export default function ApprovalCallRecording({ detail, disabled, readOnly = false, onUpdated, onBusyChange }: {
  detail: ApprovalDetail; disabled: boolean; readOnly?: boolean;
  ...
  return (
    <Card role="region" aria-label="Llamada al cliente" className="min-w-0 p-4 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {!readOnly ? <Badge tone={recording ? "positive" : "warning"}>{recording ? "Grabación cargada" : "Obligatoria"}</Badge> : null}
      </div>
      {!readOnly ? <>
      ...
      {canUpload ? <div className="mt-4 space-y-3">
        ...
        <p className="text-sm text-[var(--fp-muted)]">MP3, M4A o WAV · Máximo 10 MB. Guardar la grabación mantiene el crédito pendiente de aprobación.</p>
